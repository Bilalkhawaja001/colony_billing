...
<div class="grid">
  <div class="col-6 card">
    <h3 class="section-title">CSV HR Upload</h3>
    <div class="muted" style="margin-bottom:8px">Header: <code>company_id,name,department,designation,unit_id,active</code></div>
    <div class="toolbar">
      ...
      <button class="btn btn-primary" type="button" id="importHrCsv">Import HR CSV</button>
    </div>
  </div>
  <div class="col-6 card">
...
document.getElementById('downloadHrTemplate').onclick=()=>download('inputs_hr_template.csv','company_id,name,department,designation,unit_id,active\nE1001,Employee A,Ops,Tech,U-001,Yes\n');
document.getElementById('importHrCsv').onclick=async()=>{
  ...
  const csv=await f.text(); const res=await req('/employees/import','POST',{csv_text:csv});

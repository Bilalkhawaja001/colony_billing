...
Route::middleware(['ensure.auth', 'force.password.change', 'role:SUPER_ADMIN,BILLING_ADMIN,VIEWER'])->group(function () {
    ...
    Route::view('/electric-v1-lab/outputs', 'electric_v1.outputs');
    Route::get('/ui/electric-v1-outputs', fn () => redirect('/electric-v1-lab/outputs'));
});

Route::middleware(['ensure.auth', 'force.password.change', 'role:SUPER_ADMIN,BILLING_ADMIN,DATA_ENTRY'])->group(function () {
    Route::view('/electric-v1-lab', 'electric_v1.run');
    Route::view('/electric-v1-lab/run', 'electric_v1.run');
    Route::get('/ui/electric-v1-run', fn () => redirect('/electric-v1-lab'));
});

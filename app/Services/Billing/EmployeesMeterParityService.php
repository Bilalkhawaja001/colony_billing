...
class EmployeesMeterParityService
{
    ...
    private const HR_REQUIRED_COLUMNS = ['company_id', 'active_days'];
    ...
    public function employeesImport(array $payload): array
    {
        ...
        foreach ($rows as $idx => $row) {
            ...
            if ($this->isBlankCsvRow($row)) {
                $errors[] = ['row_no' => $line, 'company_id' => '', 'error_code' => 'BLANK_ROW', 'error_message' => 'blank row'];
                continue;
            }

            $n = $this->normalizePayload($row);
            $companyId = trim((string) ($n['company_id'] ?? ''));
            ...
            if ($missing !== []) {
                $errors[] = [
                    ...
                    'error_message' => 'Missing required: '.implode(', ', $missing),
                ];
                continue;
            }

            if (isset($seenCompanyIds[$companyId])) {
                $errors[] = [
                    'row_no' => $line,
                    'company_id' => $companyId,
                    'error_code' => 'DUPLICATE_COMPANY_ID_IN_FILE',
                    'error_message' => 'duplicate company_id in uploaded file',
                ];
                continue;
            }
            ...
            if (DB::table('employees_master')->where('company_id', $companyId)->exists()) {
                $errors[] = [
                    'row_no' => $line,
                    'company_id' => $companyId,
                    'error_code' => 'COMPANY_ID_ALREADY_EXISTS',
                    'error_message' => 'company_id already exists',
                ];
                continue;
            }
        ...
        $inserted = 0;
        DB::transaction(function () use ($normalizedRows, &$inserted) {
        ...
        return [
            ...
            'valid_rows' => count($normalizedRows),
    ...
    private function daysInMonthCycle(string $monthCycle): int
    {
        [$month, $year] = array_pad(explode('-', $monthCycle), 2, null);
        $m = (int) $month;
        $y = (int) $year;
        if ($m < 1 || $m > 12 || $y < 1900) {
            return 31;
    ...
    public function activeDaysImport(array $payload): array
    {
        $csvText = trim((string) ($payload['csv_text'] ?? ''));
        $monthCycle = trim((string) ($payload['month_cycle'] ?? ''));
        ...
        if ($monthCycle === '') {
            return ['status' => 'error', 'error' => 'month_cycle required', '_http' => 400];
        }
        ...
        $headers = array_map('trim', array_keys($rows[0] ?? []));
        $missingColumns = array_values(array_filter(self::HR_REQUIRED_COLUMNS, fn ($col) => !in_array($col, $headers, true)));
        if ($missingColumns !== []) {
            $errors = [[
                'row_no' => 1,
                'company_id' => '',
                'error_code' => 'MISSING_REQUIRED_COLUMN',
                'error_message' => 'missing required column: '.implode(', ', $missingColumns),
            ]];
            return [
                'status' => 'ok',
                'mode' => 'preview',
                ...
                'reject_report' => $errors,
            ];
        }

        $commit = (bool) ($payload['commit'] ?? false);
        $daysInMonth = $this->daysInMonthCycle($monthCycle);
        $accepted = [];
        $errors = [];
        $existing = DB::table('employees_master')->pluck('company_id')->map(fn ($v) => trim((string) $v))->all();
        $existingMap = array_fill_keys($existing, true);

        foreach ($rows as $idx => $row) {
            $rowNo = $idx + 2;
            if ($this->isBlankCsvRow($row)) {
                $errors[] = ['row_no' => $rowNo, 'company_id' => '', 'error_code' => 'BLANK_ROW', 'error_message' => 'blank row'];
                continue;
            }

            $companyId = trim((string) ($row['company_id'] ?? ''));
            $activeDaysRaw = trim((string) ($row['active_days'] ?? ''));

            if ($companyId === '') {
                $errors[] = ['row_no' => $rowNo, 'company_id' => '', 'error_code' => 'MISSING_COMPANY_ID', 'error_message' => 'missing company_id'];
                continue;
            }
            if (!isset($existingMap[$companyId])) {
                $errors[] = ['row_no' => $rowNo, 'company_id' => $companyId, 'error_code' => 'EMPLOYEE_NOT_FOUND', 'error_message' => 'employee not found in employees_master'];
                continue;
            }
            if ($activeDaysRaw === '' || !is_numeric($activeDaysRaw)) {
                $errors[] = ['row_no' => $rowNo, 'company_id' => $companyId, 'error_code' => 'ACTIVE_DAYS_INVALID', 'error_message' => 'active_days invalid'];
                continue;
            }

            $activeDays = (int) $activeDaysRaw;
            if ($activeDays < 0) {
                $errors[] = ['row_no' => $rowNo, 'company_id' => $companyId, 'error_code' => 'ACTIVE_DAYS_INVALID', 'error_message' => 'active_days invalid'];
                continue;
            }
            if ($activeDays > $daysInMonth) {
                $errors[] = ['row_no' => $rowNo, 'company_id' => $companyId, 'error_code' => 'ACTIVE_DAYS_EXCEEDS_MONTH_DAYS', 'error_message' => 'active_days exceeds month days'];
                continue;
            }

            $accepted[] = ['row_no' => $rowNo, 'company_id' => $companyId, 'active_days' => $activeDays];
        }

        $inserted = 0;
        $updated = 0;
        if ($commit) {
            DB::transaction(function () use ($accepted, $monthCycle, &$inserted, &$updated) {
                foreach ($accepted as $item) {
                    $exists = DB::table('hr_input')
                        ->where('month_cycle', $monthCycle)
                        ->where('company_id', $item['company_id'])
                        ->exists();

                    DB::table('hr_input')->updateOrInsert(
                        ['month_cycle' => $monthCycle, 'company_id' => $item['company_id']],
